...
def branded_paths() -> dict:
    """Tree-relative destination -> the file here that overwrites it.

    Needs no checkout, so check_patches.py can read it to reject a patch
    against a file this step clobbers.
    """
    icons = config.ROOT / "branding" / "icons"
    owned = {}
    if icons.is_dir():
        theme = "chrome/app/theme/chromium"
        for p in sorted(icons.glob("*.png")):
            owned[f"{theme}/{p.name}"] = p
        ...
        for density in ("default_100_percent", "default_200_percent"):
            for p in sorted((icons / density).glob("*.png")):
                owned[f"chrome/app/theme/{density}/chromium/{p.name}"] = p
    ...
    if vector.is_dir():
        for p in sorted(vector.rglob("*")):
            if p.is_file():
                owned[p.relative_to(vector).as_posix()] = p
