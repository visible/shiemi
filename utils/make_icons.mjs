...
import { MARK, paths, svg, vectorIcon, VIEWBOX } from '../branding/mark.mjs'
...
// The site draws the mark as elements rather than an image so it can take the
// page's own colour. Emitted as TypeScript because Next resolves modules under
// the app, and reaching outside it for an .mjs with no types costs more config
// than it saves.
const marker =
  ...
  paths({ size: 256 })
    ...
    .join('') +
  ']\n'
await mkdir(WEB_LIB, { recursive: true })
await writeFile(path.join(WEB_LIB, 'mark.ts'), marker)
report('lib/mark.ts', marker.length)
